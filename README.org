...
** Compilation
first install mujs (for js), vapoursynth (for python) and luajit (for lua just
in time compilation) so the meson script can see those deps and add needed build
flags before building. Then proceed to [[https://github.com/mpv-player/mpv/#compilation][compilation]] section.
...
** Todo
*** TODO Rewrite all those LUA scripts as i please
and make something like leader key (prefix) and vim-ishness possible

Additional info: [[https://www.linuxjournal.com/article/9605?page=0,0][goot article on LUA]]

*NO* js cuz mujs only provides ES5 and for me it's kinda legacy already rip
